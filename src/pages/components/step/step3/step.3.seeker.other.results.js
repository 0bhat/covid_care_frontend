...
const MyAvatar = (props) => {
    return (
        <>
            {props.type == 'free' ? <Tag color="green">Free</Tag> : <Tag color="red">Paid</Tag>}<br></br>
            <span style={{color: 'green'}}>{props.serviceType}</span>
        </>
    )
}
...
class Step2SeekerPlasmaResult extends React.Component {
    ...
    componentDidMount() {
        let dat = store.get('seeker').other.formData;
        dat.serviceTypes = store.get('seeker').other.service;
        console.log(dat);
        axios.post('http://vc.claytech.in:4433/getOtherResults', dat)
        .then((data) => {
